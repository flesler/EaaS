...
function reply(res, status, msg) {
	...
	res.writeHead(status, {'Content-Length':Buffer.byteLength(msg),'Access-Control-Allow-Origin':'*'});
	res.end(msg);
}
...
function redirect(res, redir) {
	...
	res.writeHead(302, {Location:url,'Content-Length':0,'Access-Control-Allow-Origin':'*'});
	res.end();
}