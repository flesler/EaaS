...
function reply(res, status, msg) {
	...
	if (config.production && error) {
		...
		db.storeError({url:req.url, method:req.method, time:now, status:status, msg:msg});
	}

	var type = error ? 'warn' : 'log';
	console[type]('['+type.toUpperCase()+']', req.method, req.url, status, (msg||'').split(/\r?\n/)[0]);
...
function redirect(res, redir) {
	var req = res.req;
	var url = resolveURL(req.headers.host, req.url, redir);
